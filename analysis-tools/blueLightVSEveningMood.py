...
for i in range(len(energyTimestamps)):
	...
	if(thisHour > 14): # ratings before 11.00
		sunsetTi = int((12 * 60 + 0))
		print(energyTimestamps[i] - sunsetTi * 60 * 1000)

		minBefores.append(int((12 * 60) / 5))
...
# ('slope', 3.192433759848572e-06)
# ('intercept', 80.1142779041546)
# ('r_value', 0.009953487387797465)
# ('p_value', 0.9730600471214284)
# ('std_err', 9.258369022985488e-05)
# r-squared: 9.90719111790432e-05
...
myline = np.linspace(0, 300000, 100)
plt.plot(myline, mymodel(myline), color="#000000", label="Linear regression")

plt.title('Energy level in the evening vs. accumulated 12 hours before')
plt.ylabel('Happiness')
plt.xlabel('Accumulated blue light 12 hours before (relative unit)')
plt.ylim(0, 160)
#plt.xlim(0, 1000)
plt.legend()
plt.show()
