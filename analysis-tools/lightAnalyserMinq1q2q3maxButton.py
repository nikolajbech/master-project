...
df = pandas.read_csv('../data/2020-02-10_2020-02-29_beff.csv')

#date_string = "2020-02-21 00:00:14"
#date_object = datetime.datetime.strptime(date_string, "%Y-%m-%d %H:%M:%S")
#thisMinute = int(date_object.strftime("%m"))
#print(thisMinute)
...
#while(df['Timestamp'][i] < 1587573900000):
  ...
  date = datetime.datetime.strptime(df['Timestamp'][i], "%Y-%m-%d %H:%M:%S")
...
for iddx in range(len(chunks)):
  ...
  #date = datetime.datetime.fromtimestamp(df['Timestamp'][i] / 1e3)
  date = datetime.datetime.strptime(chunks[iddx]['time'], "%Y-%m-%d %H:%M:%S")
  thisMinute = int(date.strftime("%M"))
...
if(shouldPlot):
  ...
  plt.title('Blue light from LYS Button')
